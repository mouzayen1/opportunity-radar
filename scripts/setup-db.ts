...
DROP POLICY IF EXISTS "Allow anonymous read access to opportunities" ON opportunities;
CREATE POLICY "Allow anonymous read access to opportunities"
  ON opportunities FOR SELECT
  USING (true);

DROP POLICY IF EXISTS "Allow anonymous read access to collection_runs" ON collection_runs;
...
DROP POLICY IF EXISTS "Allow service role full access to opportunities" ON opportunities;
CREATE POLICY "Allow service role full access to opportunities"
  ON opportunities FOR ALL
  USING (true);

DROP POLICY IF EXISTS "Allow service role full access to collection_runs" ON collection_runs;
...
async function setupDatabase() {
  ...
  if (!response.ok) {
    ...
    // We'll need to use the Supabase dashboard or psql for this
    console.log('\n' + '='.repeat(60));
    console.log('Please run the following SQL in your Supabase SQL Editor:');
    console.log('Dashboard URL: https://supabase.com/dashboard/project/pulzjdlyjjswtyayjdgs/sql');
